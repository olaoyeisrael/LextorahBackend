from pymongo import MongoClient
import os
from motor.motor_asyncio import AsyncIOMotorClient
from dotenv import load_dotenv

load_dotenv()

MONGO_URI = os.getenv("MONGO_URI")
client = AsyncIOMotorClient(MONGO_URI)
db = client["lextorah_materials"]
course_collection = db["courses"]
reports_collection = db["reports"]
courses_list_collection = db["courses_list"]
user_collection = db["users"]
data_collection = db["data"]
curriculum_collection = db["curriculums"]
sprints_collection = db["sprints"]
user_course_collection = db["user_courses"]
quiz_results_collection = db["quiz_results"]
live_classes_collection = db["live_classes"]
history_collection = db["history"]
starterpack_collection = db["starterpack"]
contacts_collection = db["contacts"]
assignments_collection = db["assignments"]
assignment_submissions_collection = db["assignment_submissions"]
exam_questions_collection = db["exam_questions"]
exam_results_collection = db["exam_results"]
institutional_access_collection = db["institutional_access"]
book_demos_collection = db["book_demos"]